...
let generate_kids_list () =
  List.init
    (Float.pow (Random.float 10.0 /. 3.0) 6.0 |> Int.of_float)
    (fun _ -> Random.int 10000000 + 2000000)
;;
...
let fake_posts () =
  ...
  let rec make_posts ids titles urls scores comments acc =
    ...
    | ( id :: ids_tail
      ...
      , url :: urls_tail
      , score :: scores_tail
      , comment :: comments_tail ) ->
      let post =
        { id; title; url; score; comments = comment; kids = generate_kids_list () }
      in
...
let generate_fake_comment parent_id comment_id =
  ...
  let texts =
    ...
    ; "The author makes some interesting points, but I think they're overlooking some \
       key issues."
  ...
  ; time = int_of_float (Unix.time ()) - Random.int 86400
