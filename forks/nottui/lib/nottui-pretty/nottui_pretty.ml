...
let if_flat (If_flat { then_; _ } | then_) else_ = If_flat { then_; else_ }
let internal_break i = if_flat (blank i) hardline
...
let ( ^^ ) x y =
  ...
  | lhs, rhs ->
    Cat { req = requirement lhs ++ requirement rhs; lhs; rhs; cache = Cache Uncached }
;;
...
let flat_hardline = Flat_span { prefix = Ui.empty; body = Ui.empty; suffix = Ui.empty }
...
let rec pretty_flat = function
  ...
  | Cat t ->
    (match flat_cache t.cache with
     | Some ui -> ui
     | None ->
       let result =
         let lhs = pretty_flat t.lhs
         and rhs = pretty_flat t.rhs in
         match lhs, rhs with
         | Flat_line l, Flat_line r -> Flat_line (Ui.join_x l r)
         | Flat_line l, Flat_span r -> Flat_span { r with prefix = Ui.join_x l r.prefix }
         | Flat_span l, Flat_line r -> Flat_span { l with suffix = Ui.join_x l.suffix r }
...
let nonflat_line ui = Nonflat_line { min_rem = min_int; max_rem = max_int; ui }

let nonflat_cache (Cache slot) rem wid =
  match slot with
  | Nonflat_line t' as t when t'.min_rem <= rem && rem < t'.max_rem -> Some t
  | Nonflat_span t' as t
    when t'.min_rem <= rem && rem < t'.max_rem && t'.min_wid <= wid && wid < t'.max_wid ->
    Some t
  | _ -> None
;;
...
let rec pretty (rem : int) (wid : int) = function
  ...
  | Cat t ->
    (match nonflat_cache t.cache rem wid with
     | Some ui -> ui
     | None ->
       ...
       let result =
         ...
         | Nonflat_line l ->
           ...
           (match pretty (rem - lw) wid t.rhs with
            | Nonflat_line r ->
              Nonflat_line
         ...
         | Nonflat_span l ->
           ...
           (match pretty (wid - lw) wid t.rhs with
            | Nonflat_line r ->
              Nonflat_span
                { l with
       ...
       result)
  | Nest t ->
    (match pretty rem (wid - t.indent) t.doc with
  ...
  | Group t as self ->
    ...
    then (
      ...
      | Flat_line ui -> Nonflat_line { min_rem = t.req; max_rem = max_int; ui }
      | Flat_span ui ->
        Nonflat_span
          { min_rem = t.req
          ; max_rem = max_int
      ...
      | None ->
        let result =
          ...
          | Nonflat_span ui -> Nonflat_span { ui with max_rem = mini t.req ui.max_rem }
        in
        t.cache <- Cache result;
        result)
;;
