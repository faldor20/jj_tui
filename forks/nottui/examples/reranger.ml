...
let rec dir ?(initial_path = []) ?after_width:(wref = ref 0) path =
  ...
  let body =
    W.vbox [ Lwd_table.reduce Ui.pack_y directories; Lwd_table.reduce Ui.pack_y files ]
    |> W.Scroll.v_area
  in
  let rec set_constrain constrain =
    ...
    let t =
      if constrain
      then Lwd.map ~f:(Ui.resize ~w:12) t
  ...
  let goto ?initial_path name =
    set_constrain true;
    let t =
      try dir ?initial_path ~after_width (Filename.concat path name) with
      | exn -> Lwd.return (W.string ~attr:Notty.(A.bg A.red) (Printexc.to_string exn))
    in
    ...
    @@ Ui.mouse_area
         (fun ~x:_ ~y:_ -> function
            | `Left ->
              ...
              `Handled
            | _ -> `Unhandled)
         (W.string ~attr:Notty.(A.bg (if highlight then A.lightblue else A.blue)) name)
  in
  let add_directory name =
    if name <> "" && name.[0] <> '.'
    then (
      let highlight =
        match initial_path with
    ...
    @@ Ui.mouse_area
         (fun ~x:_ ~y:_ -> function
            | `Left ->
              ...
              then
                ignore
                  (Sys.command ("xdg-open " ^ Filename.quote (Filename.concat path name))
                   : int);
              `Handled
            | _ -> `Unhandled)
         (W.string name)
  ...
  Array.iter
    (fun name ->
       ...
       try if Sys.is_directory path then add_directory name else add_file name with
       | exn ->
         let text =
           match exn with
           | Sys_error _ -> name
...
let () =
  ...
  let ui =
    W.vbox [ W.Old.main_menu_item wm "Quit" (fun () -> exit 0); dir ~initial_path "/" ]
  in
