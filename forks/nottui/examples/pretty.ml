...
let selector text f choices =
  W.Old.main_menu_item wm text (fun () ->
    Lwd.pure
    @@ Ui.vcat
         (List.map (fun choice -> W.Old.sub_entry choice (fun () -> f choice)) choices))
;;
...
let fruit =
  ...
  Lwd.join
    (Lwd.map (Lwd.get choice) ~f:(fun current -> selector current (Lwd.set choice) fruits))
;;
...
let () =
  for _ = 0 to 99 do
    List.iter
      (fun doc' -> Lwd_table.append' doc (Lwd.pure doc'))
      ...
      ; P.group
          (spring
           ^^ string "This"
           ^^ spring
           ^/^ P.group (string "is" ^^ spring ^/^ string "pretty.")
           ^^ spring)
      ; P.hardline
      ; P.ui (Ui.space 0 1)
      ; P.hardline
      ...
      (Lwd.map fruit ~f:(fun fruit ->
         P.group
           (spring
            ...
            ^^ spring
            ^/^ P.group
                  (string "like" ^^ spring ^/^ P.ui fruit ^^ spring ^/^ string "more.")
            ^^ spring)))
  done
;;
...
let doc = Lwd.join (Lwd_table.reduce (Lwd_utils.lift_monoid (P.empty, P.( ^^ ))) doc)
let contents width = Lwd.map2 ~f:P.pretty width doc

let () =
  Lwd.set base (W.h_pane (W.Scroll.area (varying_width contents)) (Lwd.pure Ui.empty));
