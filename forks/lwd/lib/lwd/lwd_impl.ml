...
let set (vx:'a var) x : unit =
  match vx with
  | (Operator ({desc = Var v; _} )as node) ->
    v.binding <- x;
    v.nextVal <- None;
    (let rec climb (parents: Any.t t_ list) =
      let new_parents : Any.t t_ list = List.fold_left (fun acc p ->
          match p with
          | Pure _ -> acc
          | Root r ->
              if Mutex.try_lock r.mutex then (
                (match r.value with
                 | Eval_some v ->
                   ...
                   r.on_invalidate v
                 | Eval_none | Eval_invalid_next -> ()
                 | Eval_progress -> r.value <- Eval_invalid_next
                );
                Mutex.unlock r.mutex;
                acc
              ) else (
                Mutex.protect r.mutex (fun () ->
                    if r.value = Eval_progress then r.value <- Eval_invalid_next
                  );
                acc
              )
          | Operator o ->
              if Mutex.try_lock o.mutex then (
                let continue =
                  match o.value with
                  | Eval_some _ -> o.value <- Eval_none; true
                  | Eval_none | Eval_invalid_next -> false
                  | Eval_progress -> o.value <- Eval_invalid_next; false
                in
                ...
                if continue then (
                  ...
                  | T0 -> acc
                  | T1 p1 -> obj_t p1 :: acc
                  | T2 (p1, p2) -> obj_t p1 :: obj_t p2 :: acc
                  | T3 (p1, p2, p3) -> obj_t p1 :: obj_t p2 :: obj_t p3 :: acc
                  | T4 (p1, p2, p3, p4) -> obj_t p1 :: obj_t p2 :: obj_t p3 :: obj_t p4 :: acc
                  | Tn t -> Array.to_list t.entries |> List.rev_append acc
                ) else acc
              ) else (
                Mutex.protect o.mutex (fun () ->
                    if o.value = Eval_progress then o.value <- Eval_invalid_next
                  );
                acc
              )
        ) [] parents in
    ...
    match node with
    | Operator o ->
        let initial_parents : Any.t t_ list =
          match o.trace with
          | T0 -> []
          | T1 p1 -> [obj_t p1]
          ...
          | Tn t -> Array.to_list t.entries
        in
        climb initial_parents
    | _ -> ()
    )
  | _ -> assert false
  ...
  let rec aux : type a b . a t_ -> b t_ -> b = fun origin ->
    ...
    | Operator t as self ->
      ...
      | Eval_none ->
          ...
          if t.value == Eval_none then aux origin self
          else (
